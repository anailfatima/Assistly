...
Search documents and FAQs for relevant content.
Combine context with user question.
Use Groq SDK to generate a grounded AI response.
...
Technology
Frontend
Vue.js, Tailwind CSS, Lucide Icons
Backend
Node.js / Express (Serverless-ready)
Database
Supabase (PostgreSQL, Storage, Auth, RLS)
AI
Groq SDK (LLM integration), RAG-style retrieval
Deployment (Recommended)
Vercel / Render / Railway (backend + frontend)
...
Backend searches relevant knowledge (docs + FAQs).
Groq LLM generates an AI response using retrieved context.
Response stored in chats table and sent to frontend.
Dashboard & Analytics
...
Learned secure environment variable managem
